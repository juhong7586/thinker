...
const PillNav = ({
  ...
  const white = pill.querySelector(`.${styles['pill-label-hover']}`);
              ...
              <li key={item.href || `item-${i}`} role="none">
                {isRouterLink(item.href) ? (
                  <Link href={item.href} legacyBehavior>
                    <a
                      role="menuitem"
                      className={[styles.pill, activeHref === item.href && styles['is-active']].filter(Boolean).join(' ')}
                      aria-label={item.ariaLabel || item.label}
                      onMouseEnter={() => handleEnter(i)}
                      onMouseLeave={() => handleLeave(i)}
                      ref={el => {
                        /* anchor ref kept on the link element if needed elsewhere */
                      }}
                    >
                      <span
          ...
          {items.map((item, i) => (
            <li key={item.href || `mobile-item-${i}`}>
              {isRouterLink(item.href) ? (
                <Link href={item.href} legacyBehavior>
                  <a
                    className={[styles['mobile-menu-link'], activeHref === item.href && styles['is-active']].filter(Boolean).join(' ')}
